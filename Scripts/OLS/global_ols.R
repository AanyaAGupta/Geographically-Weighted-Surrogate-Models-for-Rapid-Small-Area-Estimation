# ============================================================
# GLOBAL OLS WORKFLOW — Train on Training Year (80/20), Refit, Predict Transfer Year
# ============================================================
# This script performs a cross-year regression analysis using a global Ordinary Least Squares (OLS) model. 
# It:
#   1. Loads training year data (health + demographics)
#   2. Trains an OLS model on 80% of training year counties
#   3. Evaluates model on 20% holdout
#   4. Refits on all training year data
#   5. Predicts transfer year
#   6. Optionally replaces transfer year obesity and smoking with predicted values
#   7. Generates the final chronic disease predictions
# 
# NOTE:
...
#split training year data (80/20 Train/Test)
idx <- createDataPartition(df_train_year[[OUTCOME]], p = 0.8, list = FALSE)

train_data <- df_train_year[idx, ]
test_data  <- df_train_year[-idx, ]

form <- as.formula(paste(OUTCOME, "~", paste(VARS, collapse = " + ")))

fit_ols <- lm(form, data = train_data)

pred_holdout <- predict(fit_ols, newdata = test_data)
...
#refit ols on all training year data
final_ols <- lm(form, data = df_train_year)

#predict transfer year outcomes using global ols (trained on training year data)
pred_test_year <- predict(final_ols, newdata = df_test_year[, VARS, drop = FALSE])

out_test_year <- df_test_year %>%
  select(county_fips, any_of(OUTCOME)) %>%  mutate(pred_outcome = pred_test_year)

#evaluate transfer year predictions
...
# cat("\n--- GLOBAL OLS: Train Year → Transfer Year ---\n")
...
#write.csv(out_test_year, paste0("./Scripts/OLS/global_ols_t2_", OUTCOME, ".csv"), row.names = FALSE)
...
pred_obesity <- read.csv("./Scripts/OLS/global_ols_t2_per_obesity.csv") %>%
...
pred_smoking <- read.csv("./Scripts/OLS/global_ols_t2_per_smoking.csv") %>%
  select(county_fips, pred_outcome) %>%
  mutate(
    county_fips = std_fips(county_fips),
    pred_outcome = to_num(pred_outcome)
...
#   paste0("./Scripts/OLS/global_ols_t2_", OUTCOME, "_updated.csv"),
#   row.names = FALSE
# )
...
cat("\n--- GLOBAL OLS: Training Year → Transfer Year with Predicted Obesity + Smoking ---\n")
